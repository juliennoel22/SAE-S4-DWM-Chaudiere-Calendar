...
class EventService implements EventServiceInterface
{
    ...
    */
    ...
    {
        if (trim($data['title'] ?? '') === '') {
            throw new EventServiceException("Le titre de l'événement est obligatoire.");
        }
        ...
        if (!Category::find($data['category_id'])) {
            throw new EventServiceException("La catégorie spécifiée n'existe pas");
        }
        
        try {
            $event = new Event();
            $event->title = htmlspecialchars($data['title'], ENT_QUOTES, 'UTF-8');
